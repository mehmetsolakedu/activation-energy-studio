...
export const DRYAD_POLYISOPRENE_ORACLE_PATH = path.resolve(
  SCRIPT_DIRECTORY,
  '../tests/fixtures/real/dryad-polyisoprene/oracle/'
    + 'dryad_polyisoprene_decimal_oracle.py',
);

export function runDryadPolyisopreneOracle(
  oracleArguments,
  {
    runtime,
  ...
  if (result.error) {
    const error = new Error(
      `DRYAD_POLYISOPRENE_ORACLE_LAUNCH_FAILED: ${result.error.message}`,
    );
    error.code = 'DRYAD_POLYISOPRENE_ORACLE_LAUNCH_FAILED';
...
if (isMainModule()) {
  try {
    const result = runDryadPolyisopreneOracle(process.argv.slice(2));
    if (result.signal) {
      process.stderr.write(
        `${JSON.stringify({
          status: 'FAIL',
          code: 'DRYAD_POLYISOPRENE_ORACLE_TERMINATED',
    ...
    process.stderr.write(
      `${JSON.stringify({
        status: 'FAIL',
        code: error?.code ?? 'DRYAD_POLYISOPRENE_ORACLE_RUNNER_ERROR',
