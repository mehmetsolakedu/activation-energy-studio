...
export function localMacOSDiagnosticUsage() {
  return [
    'Usage:',
    '  node scripts/run-local-macos-platform-diagnostic.mjs',
    '    --output <new-or-empty-output-directory>',
    '    [--browser-executable <absolute-Google-Chrome-path>]',
    '',
    'This command runs only on a direct local macOS host.',
    'It does not emit hosted provenance, recorder input, or platform PASS evidence.',
  ].join('\n');
}

export function parseLocalMacOSDiagnosticArguments(argv) {
...
async function main() {
  const options = parseLocalMacOSDiagnosticArguments(process.argv.slice(2));
  if (!options) {
    process.stdout.write(`${localMacOSDiagnosticUsage()}\n`);
    return;
  }
