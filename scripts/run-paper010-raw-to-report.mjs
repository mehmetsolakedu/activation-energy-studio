...
export function paper010Usage() {
  return [
    'Usage:',
    '  node scripts/run-paper010-raw-to-report.mjs',
    ...
    await page.waitForFunction(
      () =>
        document.querySelector('[data-testid="alpha-grid-summary"]')
          ?.textContent?.includes('16 α noktası') === true,
      { timeout: DEFAULT_TIMEOUT_MS },
    );
    ...
    const wideMappingState = await page.evaluate(() => {
      ...
      return {
        ...
        fullTableScopeProfileVisible:
          normalizedScopeText.includes('Kaynak tablonun tamamında'),
        firstExcludedColumnVisible:
          normalizedScopeText.includes('13. temperature (575 satır; kaynak satırları 4–578)'),
        lastExcludedColumnVisible:
          normalizedScopeText.includes('36. DTG (567 satır; kaynak satırları 4–570)'),
      };
    });
    await page.click('[data-testid="apply-mapping"]');
