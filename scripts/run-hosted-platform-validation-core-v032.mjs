...
// Version-specific v0.3.2 hosted and local-diagnostic implementation core.
...
} from './platform-hosted-ci-v032.mjs';
import {
...
const APP_VERSION = process.env.AES_PLATFORM_VALIDATION_VERSION ?? '0.3.2';
if (APP_VERSION !== '0.3.2') {
  throw new Error(
    `run-hosted-platform-validation-core-v032.mjs requires AES_PLATFORM_VALIDATION_VERSION=0.3.2; received ${APP_VERSION}`,
  );
}
const IS_V032_CONTRACT = APP_VERSION === '0.3.2';
const LOCAL_MACOS_BASE_CLI_SOURCE = path.resolve(
  path.dirname(SCRIPT_PATH),
...
const LOCAL_MACOS_CLI_SOURCE = path.resolve(
  path.dirname(SCRIPT_PATH),
  IS_V032_CONTRACT
    ? 'run-local-macos-platform-diagnostic-v032.mjs'
    : 'run-local-macos-platform-diagnostic.mjs',
);
const RELEASE_NAME = `Activation-Energy-Studio-v${APP_VERSION}.html`;
const GOLDEN_NAME = 'Platform-Golden-synthetic_kas_150.csv';
const RELEASE_SOURCE = IS_V032_CONTRACT
  ? path.resolve(PROJECT_ROOT, 'release', `v${APP_VERSION}`, RELEASE_NAME)
  : path.resolve(PROJECT_ROOT, 'release', RELEASE_NAME);
const GOLDEN_SOURCE = IS_V032_CONTRACT
  ? path.resolve(PROJECT_ROOT, 'examples', 'synthetic_kas_150.csv')
  : path.resolve(PROJECT_ROOT, 'release', GOLDEN_NAME);
const CHECKSUM_SOURCE = IS_V032_CONTRACT
  ? path.resolve(
      PROJECT_ROOT,
      'evidence',
      'platform',
      `v${APP_VERSION}`,
      'PLATFORM_INPUT_SHA256SUMS.txt',
    )
  : path.resolve(PROJECT_ROOT, 'release', 'SHA256SUMS.txt');
const CANDIDATE_FREEZE_SOURCE = path.resolve(
  PROJECT_ROOT,
  'output',
  'v0.3.2-external-validation',
  'CANDIDATE_FREEZE.json',
);
const CANDIDATE_FREEZE_NAME = 'CANDIDATE_FREEZE.v0.3.2.json';
const CANDIDATE_FREEZE_SHA_SOURCE = path.resolve(
  PROJECT_ROOT,
  'output',
  'v0.3.2-external-validation',
  'CANDIDATE_FREEZE.sha256',
);
const CANDIDATE_FREEZE_SHA_NAME = 'CANDIDATE_FREEZE.v0.3.2.sha256';
...
const SELF_TEST_DOWNLOAD_NAME =
  `activation-energy-platform-self-test-v${APP_VERSION}-pass.json`;
...
const LEGACY_V020_VALIDATION_SOURCES = Object.freeze([
  Object.freeze({
    path: 'scripts/run-hosted-platform-validation.mjs',
    sha256: '629a64e83b618654106428a44033611421046fe392fcb39f2936e59e89f008f6',
  }),
  Object.freeze({
    path: 'scripts/platform-hosted-ci.mjs',
    sha256: '6fcf91e1207833e6b2bd42cb3ab9c909ed4cfef543a7cec4b85185763d6aa07d',
  }),
  Object.freeze({
    path: 'scripts/run-local-macos-platform-diagnostic.mjs',
    sha256: '1a87bcf3768c34f010bb82e00ecba42d019fa9184d6b8d5e88c922dc1bdeeca0',
  }),
]);
const LEGACY_V032_VALIDATION_SOURCES = Object.freeze([
  Object.freeze({
    path: 'scripts/run-hosted-platform-validation.mjs',
    sha256: '29ac23fe5429b25c9edfa1f4b555455e5cebbeda2a9823c50a966b2d20d4e979',
  }),
  Object.freeze({
    path: 'scripts/platform-hosted-ci.mjs',
    sha256: '2973ec2e7ccd876b967a32127a33b006341f85880c016dbf8791ff1c566d5f63',
  }),
  Object.freeze({
    path: 'scripts/run-local-macos-platform-diagnostic.mjs',
    sha256: 'a93044972576ba82259e6f76abf1bae6904815f8b6beb7bb14a8054cf158c535',
  }),
  Object.freeze({
    path: 'scripts/run-local-macos-platform-diagnostic-v032.mjs',
    sha256: 'a5faf08ad380fcbb019ac23c878f2ae0a8374b210b11cd4523842318210f6f20',
  }),
]);
...
const LOCAL_MACOS_EXECUTION_PROFILE = Object.freeze({
  ...
  nextRequiredAction:
    'No further action is required for the solo Research Preview closeout. If AC-PLAT-01, AC-PLAT-02, or cross-platform compatibility is later pursued, retain this run as local technical evidence and collect separately observed macOS, Windows 11, and Ubuntu runs under the optional external platform protocol.',
});
...
function usage() {
  return [
    'Usage:',
    '  node scripts/run-hosted-platform-validation-v032.mjs',
  ...
  for (const [index, line] of text.split(/\r?\n/u).entries()) {
    ...
    if (!match) {
      fail(
        ...
        `The v${APP_VERSION} platform checksum index line ${index + 1} is invalid.`,
      );
    }
...
async function readValidatedCandidateFreeze() {
  let freeze;
  try {
    freeze = JSON.parse(await readFile(CANDIDATE_FREEZE_SOURCE, 'utf8'));
  } catch (error) {
    fail(
      'CANDIDATE_FREEZE_INVALID',
      `Cannot read the v0.3.2 candidate freeze: ${error?.message ?? String(error)}.`,
    );
  }
  if (
    !freeze ||
    ...
    freeze.releaseVersion !== APP_VERSION ||
    freeze.status !== 'HASH_LOCKED_EXTERNAL_VALIDATION_IN_PROGRESS'
  ) {
    fail(
      'CANDIDATE_FREEZE_INVALID',
      'Candidate freeze identity or status is not the locked v0.3.2 contract.',
    );
  }
  const candidate = freeze.candidate;
  if (
    !candidate ||
    candidate.path !== `release/v${APP_VERSION}/${RELEASE_NAME}` ||
    ...
    fail(
      'CANDIDATE_FREEZE_INVALID',
      'Candidate freeze does not contain the exact v0.3.2 release descriptor.',
    );
  }
  if (
    ...
    fail('CANDIDATE_FREEZE_INVALID', 'Candidate freeze integrity policy is not fail-closed.');
  }
  ...
  for (const gateId of ['AC-PLAT-01', 'AC-PLAT-02']) {
    ...
    if (gate?.lane !== 'cross-platform-runtime' || gate?.status !== 'EXTERNAL_OPEN') {
      fail('CANDIDATE_FREEZE_INVALID', `${gateId} is not EXTERNAL_OPEN in the freeze.`);
    }
  }
  if (!Array.isArray(freeze.boundArtifacts) || freeze.boundArtifacts.length < 7) {
    fail('CANDIDATE_FREEZE_INVALID', 'Candidate freeze omits required bound artifacts.');
  }
  ...
  for (const artifact of freeze.boundArtifacts) {
    if (
      ...
      fail('CANDIDATE_FREEZE_INVALID', 'Candidate freeze has an invalid bound-artifact descriptor.');
    }
    ...
    if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) {
      fail('CANDIDATE_FREEZE_INVALID', `Unsafe freeze-bound path: ${artifact.path}.`);
    }
    let metadata;
    try {
      metadata = await stat(artifactPath);
    } catch {
      fail('CANDIDATE_FREEZE_STALE', `Freeze-bound artifact is missing: ${artifact.path}.`);
    }
    if (
      !metadata.isFile() ||
      ...
      fail('CANDIDATE_FREEZE_STALE', `Freeze-bound artifact changed: ${artifact.path}.`);
    }
  }
  if (!boundRoles.has('release-html')) {
    fail('CANDIDATE_FREEZE_INVALID', 'Candidate freeze omits the release-html binding.');
  }
  const bytes = await readFile(CANDIDATE_FREEZE_SOURCE);
  const checksumBytes = await readFile(CANDIDATE_FREEZE_SHA_SOURCE);
  const checksumMatch = /^([a-f0-9]{64})  CANDIDATE_FREEZE\.json\s*$/u.exec(
    checksumBytes.toString('utf8'),
  );
  if (!checksumMatch || checksumMatch[1] !== sha256Bytes(bytes)) {
    fail(
      'CANDIDATE_FREEZE_INVALID',
      'Candidate-freeze checksum sidecar does not match the freeze bytes.',
    );
  }
  return Object.freeze({
    bytes,
...
async function verifyAndCopyLockedInputs(outputDirectory) {
  for (const source of [
    CHECKSUM_SOURCE,
    ...(IS_V032_CONTRACT
      ? [CANDIDATE_FREEZE_SOURCE, CANDIDATE_FREEZE_SHA_SOURCE]
      : []),
    RELEASE_SOURCE,
    GOLDEN_SOURCE,
  ]) {
  ...
  const freeze = IS_V032_CONTRACT
    ? await readValidatedCandidateFreeze()
    : null;
  ...
  if (!expectedRelease || !expectedGolden) {
    fail(
      'RELEASE_CHECKSUM_INVALID',
      `The v${APP_VERSION} platform checksum index does not lock both validation inputs.`,
    );
  }
  ...
  if (
    ...
    (IS_V032_CONTRACT && actualRelease !== freeze.candidate.sha256) ||
    (IS_V032_CONTRACT &&
      (await stat(RELEASE_SOURCE)).size !== freeze.candidate.bytes) ||
    actualGolden !== expectedGolden
  ) {
  ...
  const candidateFreezePath = IS_V032_CONTRACT
    ? path.resolve(outputDirectory, CANDIDATE_FREEZE_NAME)
    : null;
  const candidateFreezeChecksumPath = IS_V032_CONTRACT
    ? path.resolve(outputDirectory, CANDIDATE_FREEZE_SHA_NAME)
    : null;
  await copyFile(RELEASE_SOURCE, releasePath);
  await copyFile(GOLDEN_SOURCE, goldenPath);
  if (IS_V032_CONTRACT) {
    await writeFile(candidateFreezePath, freeze.bytes);
    await writeFile(candidateFreezeChecksumPath, freeze.checksumBytes);
  }
  if (
    (await sha256File(releasePath)) !== expectedRelease ||
    (await sha256File(goldenPath)) !== expectedGolden ||
    (IS_V032_CONTRACT &&
      (await sha256File(candidateFreezePath)) !== sha256Bytes(freeze.bytes)) ||
    (IS_V032_CONTRACT &&
      (await sha256File(candidateFreezeChecksumPath)) !==
        sha256Bytes(freeze.checksumBytes))
  ) {
  ...
  return Object.freeze({
    ...
    candidateFreezeSha256: IS_V032_CONTRACT
      ? sha256Bytes(freeze.bytes)
      : null,
  });
}
...
async function executeAutomatedBrowserRun(
  ...
  try {
    ...
    await recordAction(timeline, marker, 'upload', async () => {
      ...
      if (IS_V032_CONTRACT) {
        await waitForEnabled(page, '[data-testid="confirm-interpretation"]');
        const interpretationConfirmed = await page.$eval(
        ...
        if (!interpretationConfirmed) {
          fail(
            'UI_CONFIRMATION_CONTRACT_FAILED',
            'The v0.3.2 interpretation confirmation did not commit.',
          );
        }
      }
  ...
  const currentFreeze = IS_V032_CONTRACT
    ? await readValidatedCandidateFreeze()
    : null;
  if (
    IS_V032_CONTRACT &&
    (sha256Bytes(currentFreeze.bytes) !== lockedInputs.candidateFreezeSha256 ||
      currentFreeze.candidate.sha256 !== lockedInputs.releaseSha256)
  ) {
    fail(
      'CANDIDATE_FREEZE_CHANGED_DURING_RUN',
      'Candidate freeze changed during the controlled browser interval.',
    );
  }
  ...
  const metadata = {
    ...
    ...(IS_V032_CONTRACT
      ? {
          candidateFreeze: {
            record: await descriptor(
              lockedInputs.candidateFreezePath,
              options.outputDirectory,
              { includeBytes: true },
            ),
            checksum: await descriptor(
              lockedInputs.candidateFreezeChecksumPath,
              options.outputDirectory,
              { includeBytes: true },
            ),
          },
    ...
    retainedFiles: {
      rawCdpEvents:
        IS_V032_CONTRACT && profile.kind === 'hosted'
          ? await descriptor(rawEventsPath, options.outputDirectory, {
              includeBytes: true,
            })
          : path.basename(rawEventsPath),
      ...(draftManifestPath
...
const LOCAL_DIAGNOSTIC_FILE_ROLES = Object.freeze({
  ...
  ...(IS_V032_CONTRACT
    ? {
        [CANDIDATE_FREEZE_NAME]: 'locked_external_validation_candidate_freeze',
        [CANDIDATE_FREEZE_SHA_NAME]: 'candidate_freeze_checksum_sidecar',
      }
    : {}),
...
export async function verifyLocalMacOSDiagnosticBundle(outputDirectory) {
  ...
  const expectedValidationSources = [
    {
      path: 'scripts/run-hosted-platform-validation-core-v032.mjs',
      sha256: await sha256File(SCRIPT_PATH),
    },
    {
      path: 'scripts/platform-hosted-ci-v032.mjs',
    ...
    ...(IS_V032_CONTRACT
      ? [
          {
            path: 'scripts/run-local-macos-platform-diagnostic.mjs',
            sha256: await sha256File(LOCAL_MACOS_BASE_CLI_SOURCE),
          },
        ]
      : []),
    {
      path: IS_V032_CONTRACT
  ...
  const matchesValidationSources = (expectedSources) =>
    Array.isArray(metadata.provenance.validationSources) &&
    metadata.provenance.validationSources.length === expectedSources.length &&
    expectedSources.every(
      (expected, index) =>
        metadata.provenance.validationSources[index]?.path === expected.path &&
        metadata.provenance.validationSources[index]?.sha256 === expected.sha256,
    );
  const historicalValidationSources = IS_V032_CONTRACT
    ? LEGACY_V032_VALIDATION_SOURCES
    : LEGACY_V020_VALIDATION_SOURCES;
  if (
    !matchesValidationSources(expectedValidationSources) &&
    !matchesValidationSources(historicalValidationSources)
  ) {
    fail(
  ...
  if (
    IS_V032_CONTRACT &&
    (metadata.candidateFreeze?.record?.path !== CANDIDATE_FREEZE_NAME ||
      metadata.candidateFreeze?.record?.sha256 !==
        manifestEntries.get(CANDIDATE_FREEZE_NAME)?.sha256 ||
      metadata.candidateFreeze?.record?.sha256 !==
        sha256Bytes(currentFreeze.bytes) ||
      metadata.candidateFreeze?.checksum?.path !== CANDIDATE_FREEZE_SHA_NAME ||
      metadata.candidateFreeze?.checksum?.sha256 !==
        manifestEntries.get(CANDIDATE_FREEZE_SHA_NAME)?.sha256 ||
      metadata.candidateFreeze?.checksum?.sha256 !==
        (await sha256File(CANDIDATE_FREEZE_SHA_SOURCE)))
  ) {
    fail(
      'LOCAL_DIAGNOSTIC_METADATA_INVALID',
      'Metadata candidate freeze does not match the retained and current freeze bytes.',
    );
  }
  return Object.freeze({
